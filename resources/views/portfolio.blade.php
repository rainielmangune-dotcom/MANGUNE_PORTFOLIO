...
@if(session('status'))
<div style="max-width:500px; margin:auto; background:#2b2b2b; padding:10px 20px; border-radius:8px; color:#4CAF50; text-align:center; margin-top:10px;">
    {{ session('status') }}
</div>
@endif
        ...
        <ul>
          ...
          <li>
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button style="background:none; color:#ddd; border:none; cursor:pointer;">Logout</button>
            </form>
          </li>
        </ul>
      </nav>
